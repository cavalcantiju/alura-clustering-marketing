...
#coletamos todos os objetos que deixamos como arquivos .pkl
...
def processar_prever (df):
    # Todas as linhas abaixo devem ter o mesmo recuo (4 espaços)
    ...
    # Corrigido: agora esta linha está alinhada com as anteriores
    dados_escalados = scaler.transform(dados)

    cluster = kmeans.predict(dados_escalados)

    return cluster#Ela retorna um Array do NumPy contendo os rótulos (labels) dos clusters.
#Se você passar um DataFrame com 3 pessoas para essa função, ela vai te
# devolver algo assim: array([0, 2, 1])
